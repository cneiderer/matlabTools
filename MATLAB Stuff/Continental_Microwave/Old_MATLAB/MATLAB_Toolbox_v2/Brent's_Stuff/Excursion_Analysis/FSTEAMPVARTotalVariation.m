...
KU_NCDL_IL    = 1; %toggles Ku NCDL IL spec (14.54-14.79)
...
noplot        = 1;  %turns off manual window display
...
if exc==0
    ...
    xlabel('# Window Steps');
    ylabel({'Amplitude (dB)',['(Max Var = ',num2str(max(w1MaxVar),2),')']});
    axis([1 length(w1MaxVar) 0 RPL1+.2+exc])
    legend('Specified Variation','Measured Data','Location','EastOutside')
    grid on
    ...
    xlabel('# Window Steps');
    ylabel({'Amplitude (dB)',['(Max Var = ',num2str(max(w2MaxVar),2),')']});
    ...
    ylabel({'Amplitude (dB)',['(Max Var = ',num2str(max(w1MaxVar),2),')']});
    axis([1 length(w1MaxVar) 0 RPL1+.2+exc])
    legend('Specified Variation','Allowed Excursion','Measured Data','Location','EastOutside')
    grid on
