function [] = waveguide_filter_design_ver3(varargin)

%
% waveguide_filter_design_ver3.m
%
% Inputs:
%   passband_ripple         -> [dB]
%   center_freq             -> [GHz]
...
%   Date: 1/14/2009
%   Change 3.1:
%       HFSS_Coord consists only of boxes (x,y,z,dx,dy,dz) all drawn
%       relative to the global coorinate system
%   
%   Date: 1/15/2009
%   Change 3.2:
...
filter_length=center_freqLambda/4;
HFSS_coord(end+1,:)={'inputair',num2str(-waveguide_a/2),'0','0',num2str(waveguide_a),num2str(filter_length),num2str(waveguide_b)};

for cc=1:length(iris_values)
    
    HFSS_coord(end+1,:)={['iris_',num2str(cc-1),',',num2str(cc)],num2str(-iris_values(cc)/2),num2str(filter_length),'0',num2str(iris_values(cc)),num2str(iris_thickness),num2str(waveguide_b)};
    filter_length=filter_length+iris_thickness;
    
    if cc<=length(resonator_lengths)
        HFSS_coord(end+1,:)={['res',num2str(cc)],num2str(-waveguide_a/2),num2str(filter_length),'0',num2str(waveguide_a),num2str(resonator_lengths(cc)),num2str(waveguide_b)};
        filter_length=filter_length+resonator_lengths(cc);
    end
        
end

HFSS_coord(end+1,:)={'outputair',num2str(-waveguide_a/2),num2str(filter_length),'0',num2str(waveguide_a),num2str(center_freqLambda/4),num2str(waveguide_b)};
