...
if nargin==1
    ...
    cti_file_list=get_cti_files;
end

test=1;

%%
if isempty(cti_file_list)
    
    error('No *.cti files were selected for analysis')
    ...
    for zz=1:length(cti_file_list)

        % Record file name & path
        system_data(zz).fullfile=cti_file_list{zz,1};
        system_data(zz).filepath=cti_file_list{zz,2};
        system_data(zz).filename=cti_file_list{zz,3};
        system_data(zz).system_name=cti_file_list{zz,4};
        system_data(zz).data_type=cti_file_list{zz,5};
        system_data(zz).band_name=cti_file_list{zz,6};
        system_data(zz).center_freq=cti_file_list{zz,7};
        system_data(zz).polarization=cti_file_list{zz,8};
    ...
    for xx=1:length(system_data)
        ...
        % Increment Waitbar
        waitbar(xx/length(system_data),h,['Analyzing data from system *.cti files,',...
