...
while xx
    ...
    % Select set of *.cti files for analysis
    [load_filenames,loadfiles_path]=uigetfile('*.cti','Select the *.cti files to be analyzed:',...
        'MultiSelect','on');
...
%     save_or_not=save_file_list;
    
    save(fullfile('S:\Curtis_Neiderer\MATLAB_Toolbox\',['selected_cti_file_list_',datestr(now,1),'.mat']),'cti_file_list');

else
   disp('-- No *.cti files were selected for analysis') 
end
...
yy=1;
while yy
    more_files=input('Would you like to select another set of *.cti files (Y/N)? ','s');
...
% 
function [decision] = save_file_list
...
while yy
    ...
    if strcmpi(collected_input,'Y') || strcmpi(collected_input,'Yes') || isempty(collected_input)
        ...
        [list_name,list_path]=uiputfile('*.mat','Save detailed file list as *.mat file',['selected_cti_file_list_',datestr(now,1),'.mat']);
