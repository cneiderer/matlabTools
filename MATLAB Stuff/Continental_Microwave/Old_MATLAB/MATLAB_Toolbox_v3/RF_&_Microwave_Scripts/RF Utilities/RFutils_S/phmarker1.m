function Sinterp=phmarker1(Snn,Freq,Zo,Markers,Mnumber)
% Phase Delay Marker
%
% Draws a single marker on Phase Delay plot at frequency specified
...
hold on;
L1=angle(Ymarker).*180./pi;   % Phase Delay (Deg)
...
if Mnumber==1
 ...
 t1a=sprintf('%5.2f Deg',real(L1(1)));
 textsc(RHSx,.90,t1);
 textsc(RHSx,.85,t1a);
end
...
if Mnumber==2
 ...
 t2a=sprintf('%5.2f Deg',real(L1(1)));
 textsc(RHSx,.70,t2);
 textsc(RHSx,.65,t2a);
end
...
if Mnumber==3
 ...
 t3a=sprintf('%5.2f Deg',real(L1(1)));
 textsc(RHSx,.50,t3);
 textsc(RHSx,.45,t3a);
end
...
if Mnumber==4
 ...
 t4a=sprintf('%5.2f Deg',real(L1(1)));
 textsc(RHSx,.30,t4);
 textsc(RHSx,.25,t4a);
end
