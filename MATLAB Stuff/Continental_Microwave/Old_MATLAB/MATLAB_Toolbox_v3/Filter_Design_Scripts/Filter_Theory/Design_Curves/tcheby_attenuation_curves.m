function [] = tcheby_attenuation_curves(varargin)
...
for ii=1:filter_order
    ...
    for jj=1:length(freq_ratio)
        ...
        attenuation=10*log10(1+(epsilon*(cos(ii*acos(freq_ratio(jj))))^2));
%         attenuation=10*log10(1+(epsilon*(cosh(ii*acosh(freq_ratio(jj))))^2));
