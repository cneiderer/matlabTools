...
filetype_list=['14.615*RH*.cti;',...
               '15.25*RH*.cti;',...
               '14.665*RH*.cti;',...
               '15.19*RH*.cti;',...
               '9.85*RH*.cti;',...
               '14.125*.cti;'];
[tx_files,tx_path]=uigetfile(filetype_list,'Select the NB "TX" *.cti files to be analyzed:','MultiSelect','on');

% Test to make sure you selected a set of files
if ~iscell(tx_files) && ~isstr(tx_files)
    ...
    if isstr(tx_files)
        tx_filepaths{1,1}=fullfile(tx_path,tx_files);
        disp(['--',fullfile(tx_path,tx_files)])
    else
...
filetype_list=['14.615*RH*.cti;',...
               ...
               '14.665*RH*.cti;',...
               '15.19*RH*.cti;'];
[WB_tx_files,WB_tx_path]=uigetfile(filetype_list,'Select the WB "TX" *.cti files to be analyzed:','MultiSelect','on');

% Test to make sure you selected a set of files
if ~iscell(WB_tx_files) && ~isstr(WB_tx_files)
    warning('You did not select a set of WB "TX" *.cti files.')
    WB_tx_files=[];
else
    WB_tx_filepaths={};
	disp('WB "TX" *.cti files selected: ')
    if isstr(WB_tx_files)
        WB_tx_filepaths{1,1}=fullfile(WB_tx_path,WB_tx_files);
        disp(['--',fullfile(WB_tx_path,WB_tx_files)])
    else
...
filetype_list=['14.615*RH*.cti;',...
               ...
               '10.3*RH*.cti;',...
               '11.85*.cti;'];
[rx_files,rx_path]=uigetfile(filetype_list,'Select the NB "RX" *.cti files to be analyzed:','MultiSelect','on');

% Test to make sure you selected a set of files
if ~iscell(rx_files) && ~isstr(rx_files)
    ...
    if isstr(rx_files)
        rx_filepaths{1,1}=fullfile(rx_path,rx_files);
        disp(['--',fullfile(rx_path,rx_files)])
    else
...
filetype_list=['14.615*RH*.cti;',...
               ...
               '14.665*RH*.cti;',...
               '15.19*RH*.cti;'];
[WB_rx_files,WB_rx_path]=uigetfile(filetype_list,'Select the WB "RX" *.cti files to be analyzed:','MultiSelect','on');

% Test to make sure you selected a set of files
if ~iscell(WB_rx_files) && ~isstr(WB_rx_files)
    warning('You did not select a set of WB "RX" *.cti files.')
    WB_rx_files=[];
else
    WB_rx_filepaths={};
    disp('WB "RX" *.cti files selected: ')
    if isstr(WB_rx_files)
        WB_rx_filepaths{1,1}=fullfile(WB_rx_path,WB_rx_files);
		disp(['--',fullfile(WB_rx_path,WB_rx_files)])
    else
...
% NB "Tx"
if exist('tx_filepaths','var') && iscell(tx_filepaths)
...
% WB "Tx"
if exist('WB_tx_filepaths','var') && iscell(WB_tx_filepaths)
...
% NB "Rx"
if exist('rx_filepaths','var') && iscell(rx_filepaths)
...
% WB "Rx"
if exist('WB_rx_filepaths','var') && iscell(WB_rx_filepaths)
...
% save(['S:\Curtis_Neiderer\MATLAB_Toolbox\',sys_name,'_measurement_data_',datestr(now,1),'.mat'],'tx_data','WB_tx_data','rx_data','WB_rx_data');
save(['C:\Revamped_MATLAB_Toolbox\Results\',sys_name,'_measurement_data_',datestr(now,1),'.mat'],'tx_data','WB_tx_data','rx_data','WB_rx_data');
