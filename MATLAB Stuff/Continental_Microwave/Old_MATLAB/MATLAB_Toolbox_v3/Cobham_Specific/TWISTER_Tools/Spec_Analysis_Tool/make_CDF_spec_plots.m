...
%   Date: 12/xx/2008
%   Comment: 
%       This will probably need to be re-written to pull data from struct 
%       when RF_spec_analysis_mod.xls is changed to store data differently.  
%   
%   Date: 1/14/2009
%   Change 1.1:
%       Edit YTickLabel on plots.  Changed the value to an empty string.
%       set(gca,'YTickLabel','');
%
...
%                     xlabel('');
            ...
            % Change YTickLabel to empty string
            set(gca,'YTickLabel','')
        ...
        if ~exist('saveas_ppt','var')
            [ppt_file,ppt_path]=uiputfile('*.ppt','Save file as: ',['CDF_spec_plots_',datestr(now,1),'.ppt']);
