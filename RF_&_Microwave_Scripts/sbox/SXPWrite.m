...
s_digits = 8; f_digits = 6; r_digits = 16;
...
if order > 2
   for i = 1:N
      ...
      word = SXPSXPstrfit(candidate, r_digits);
      for j = 1:order
         candidate = num2str(real(s(1,j,i)), s_digits);
         word = [word SXPstrfit(candidate, r_digits)];
...
elseif order == 2 
   for i = 1:N
      word = [SXPstrfit(num2str(freq(i)*unit_adj), r_digits) SXPstrfit(num2str(real(s(1,1,i)), s_digits), r_digits) SXPstrfit(num2str(imag(s(1,1,i)), s_digits), r_digits) SXPstrfit(num2str(real(s(2,1,i)), s_digits), r_digits)   SXPstrfit(num2str(imag(s(2,1,i)), s_digits), r_digits) SXPstrfit(num2str(real(s(1,2,i)), s_digits), r_digits)  SXPstrfit(num2str(imag(s(1,2,i)), s_digits), r_digits) SXPstrfit(num2str(real(s(2,2,i)), s_digits), r_digits) SXPstrfit(num2str(imag(s(2,2,i)), s_digits), r_digits)];
      fprintf(fid, '%s\n', word);
   end;
elseif order == 1
   for i = 1:N
      word = [SXPstrfit(num2str(freq(i)*unit_adj), r_digits) SXPstrfit(num2str(real(s(i)), s_digits), r_digits)  SXPstrfit(num2str(imag(s(i)), s_digits), r_digits)];
      fprintf(fid, '%s\n', word);
   end;
end;
