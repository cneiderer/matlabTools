...
%
% Description:
%   Loads system data and specs. Writes specs and system data to workbook,
%   comparing values for appropriate formatting (green=passed,
%   yellow=passed marginally, red=failed).
%
% Inputs:
%   system_data     ->  path to system_data_*.mat file created in
%                       spec_analysis_xls.m
%
% Outputs:
%   spec_analysis_*.xls     ->  formatted spreadsheet of all the system data
%
% Author:
...
%   Date: 1/14/2008
%   Change 1.1:
%       Added an empty results check to skip system data that was not
%       properly analyzed and stored in compiled_system_data.mat
%
%           if ~isempty(system_data(ii).results)
%               "Write data to workbook"
%           end
%       
%       May need to make this more robust in the future if some of the
%       results are properly analyzed but a few were not. But I'll cross
...
while xx
    ...
    elseif strcmpi(existing_xls,'N') || strcmpi(existing_xls,'No') || isempty(existing_xls)
        ...
        [new_xls,new_xlspath]=uiputfile('*.xls','Save your spec analysis spreadsheet as: ',['spec_analysis_',datestr(now,1),'.xls']);
        if new_xls==0
