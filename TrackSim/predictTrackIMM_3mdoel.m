function [xm, pmMatrix, mProb,cov_ruv] = predictTrackIMM ...
    ...
    xm, pmMatrix, mProb, deltaTime,constants,radar,flags,man_flags);
...
xmTemp(:,1) =  mixProb(1,1)*xm(:,1) + mixProb(2,1)*xm(:,2) + mixProb(3,1)*xm(:,3);
xmTemp([7 8 10],1)=0;

xmTemp(:,2) = mixProb(1,2)*xm(:,1) + mixProb(2,2)*xm(:,2) + mixProb(3,2)*xm(:,3);
xmTemp([7 8 10],2)=xm([7 8 10],2);

xmTemp(:,3) =  mixProb(1,3)*xm(:,1) + mixProb(2,3)*xm(:,2) + mixProb(3,3)*xm(:,3);
xmTemp([7 8 10],1)=0;
...
%     end
% end
% pmMatrixTemp(7:10,7:10,2)=pmMatrix(7:10,7:10,2);
% 
...
deltaX23=(xm(:,3) - xmTemp(:,2));
deltaX23([7 8 10])=0;

deltaX32=(xm(:,2) - xmTemp(:,3));
deltaX32([7 8 10])=0;

        pmMatrixTemp(:,:,1) =mixProb(1,1)*(pmMatrix(:,:,1) + (xm(:,1) - xmTemp(:,1))*(xm(:,1) - xmTemp(:,1))') +  mixProb(2,1)*(pmMatrix(:,:,2) + deltaX12*deltaX12') +  mixProb(3,1)*(pmMatrix(:,:,3) + (xm(:,3) - xmTemp(:,1))*(xm(:,3) - xmTemp(:,1))');
        pmMatrixTemp(:,:,2) =mixProb(1,2)*(pmMatrix(:,:,1) + deltaX21*deltaX21') +  mixProb(2,2)*(pmMatrix(:,:,2) + (xm(:,2) - xmTemp(:,2))*(xm(:,2) - xmTemp(:,2))') +  mixProb(3,2)*(pmMatrix(:,:,3) + deltaX23*deltaX23') ;
        pmMatrixTemp(:,:,3) =mixProb(1,3)*(pmMatrix(:,:,1) + (xm(:,1) - xmTemp(:,3))*(xm(:,1) - xmTemp(:,3))')  +  mixProb(2,3)*(pmMatrix(:,:,2)  + deltaX23*deltaX23') + mixProb(3,3)*(pmMatrix(:,:,3) + (xm(:,3) - xmTemp(:,3))*(xm(:,3) - xmTemp(:,3))')  ;
...
for jj = 1:nm
    ...
    elseif jj==2 %% booster state group
        %initialize sg, propagate
        input_group.track_type=3;
         input_group.search_source=8;
    output_group = propagate_group(input_group, deltaTime, constants, radar, flags);
    elseif jj==3 %%maneuver
     input_group.track_type=2;
        input_group.search_source=8;
