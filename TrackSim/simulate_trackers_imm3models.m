function [constants radar truth Tlower Tupper kf amm imm detection]=simulate_trackers(constants, radar, truth)
%   UNCLASSIFIED
%
%   DESCRIPTION
...
nm=3;
ns=10;
probTranMatrix = [0.99  0.01 0;  0.04  0.95 0; 0.05  0.95 0];
...
xm=[[current6sgroup_IMM.pos_rrc; current6sgroup_IMM.vel_rrc; current6sgroup_IMM.acceleration] [current10sgroup_IMM.pos_rrc; current10sgroup_IMM.vel_rrc; current10sgroup_IMM.acceleration] [current6sgroup_IMM.pos_rrc; current6sgroup_IMM.vel_rrc; current6sgroup_IMM.acceleration]];
x=xm(:,1);
pmMatrix(1:10,1:10,1)=current6sgroup_IMM.cov_rrc(1:10,1:10);
pmMatrix(1:10,1:10,2)=current10sgroup_IMM.cov_rrc(1:10,1:10);
pmMatrix(1:10,1:10,3)=current6sgroup_IMM.cov_rrc(1:10,1:10);
...
for dd=3:length(detection.time)
    ...
    if c_det.time<490%% condition to switch from 10-state/6-state to 6-state/maneuver
        ...
        score = score + computeLogLikelihoodRatio(propmangroup,nextmangroup,prop6sgroup,next6sgroup,c_det);
        if score > Tupper  % maneuver is confirmed
            amm_model='man';
        ...
        elseif score < Tlower % exo is confirmed.  reset maneuver filter and reprocess current detection
            amm_model='exo';
    ...
    xm(7:10,[1 3])=0;
    pmMatrix(1:6,7:10,[1 3])=0;
    pmMatrix(7:10,1:10,[1 3])=0;

    [xm, pmMatrix, mProb,cov_ruv] = predictTrackIMM ...
        (ns, nm, probTranMatrix, ...
        xm, pmMatrix, mProb, dt,constants,radar,flags,man_flags);
    ...
    if ~in_man_filter & man_det & c_det.time>380
        in_man_filter=1;
