function h = waitbarTimeLeft(X,message,varargin)
%  WAITBAR a modified version of MATLAB's waitbar function.

%     H = WAITBARTIMELEFT(X,'message') creates and displays a waitbar of fractional 
%           length X.  The handle to the waitbar figure is returned in H.
%           X should be between 0 and 1. 
%  
%     WAITBARTIMELEFT(X) will set the length of the bar in the most recently
%           created waitbar window to the fractional length X.
%  
%     WAITBARTIMELEFT(X,H) will set the length of the bar in waitbar H
%           to the fractional length X.
%  
%     WAITBARTIMELEFT(X,H,'message') will update the message text in
%           the waitbar figure, in addition to setting the fractional
%           length to X.
%  
%     WAITBARTIMELEFT is typically used inside a FOR loop that performs a
%           lengthy computation.  
%  
%     Example:
%         h = waitbarTimeLeft(0,'Please wait...');
%         for i=1:1000,
%             % computation here %
%             waitbar(i/1000,h)
%         end
...
% 1 - GATHER THE INPUT
    if nargin == 1;
        ...
        message = '';
    elseif isnumeric(X) & ischar(message)
        h = [];
    elseif isnumeric(X) & ishandle(message) & isempty(varargin);
        h = message;
    elseif isnumeric(X) & ishandle(message) & ~isempty(varargin);
        h = message; message = varargin{1};
    else
...
% BUILDWAITBAR constructs the figure containing the waitbar
 ...
 % 1 - SET WINDOW SIZE AND POSITION 
    ...
    % 1.2 - Define the waitbar position
        winwidth = 500;           % Width of timebar window
...
% 2 - OPEN FIGURE AND SET PROPERTIES  
    ...
    h = figure('menubar','none','numbertitle','off',...
        ...
        'tag','TMWWaitbar','IntegerHandle','off');                               
...
% 3 - INITILIZE THE PROGESS BAR
    ...
    set(h,'userdata',userdata)                          % Store data in the created figure
    updatewaitbar(h,X,message);                         % Updates waitbar if created with X~=0
